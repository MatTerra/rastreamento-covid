...
def select_local_checkin():
    ...
    try:
        ...
        while True:
            ...
            if total == 0:
                print(
                    f"{bcolors.WARNING}Não há locais disponíveis para "
                    f"checkin\n"
            ...
            while option != 'x' and option != 's':
                ...
                if option == '[':
                    ...
                    if option == "C":
                        system("clear")
                        if page == pages and selected_local == total % \
                                itens_per_page - 1:
                            break
...
def create_checkin(user: Usuario):
    ...
    while True:
        print(f"{bcolors.BOLD}"
              ...
              f"(digite uma data no formato dd/mm/yy HH:MM. Exemplo: "
              f"03/02/20-16:05)"
              f"{bcolors.ENDC}\n")
        try:
            inicio_checkin = datetime.strptime(input(">> ").strip(),
                                               datetime_format)
        except KeyboardInterrupt:
            return
        except Exception as e:
            print(f"{bcolors.WARNING}"
                  f"Ops, a data que você digitou está no formato "
                  f"errado.\nPor favor, digite novamente conforme o formato "
                  f"dado."
                  f"{bcolors.ENDC}\n")
        else:
            break
    ...
    while True:
        print(f"{bcolors.BOLD}"
              ...
              f"{bcolors.ENDC}\n")
        try:
            final_checkin = datetime.strptime(input(">> ").strip(),
                                              datetime_format)

            if final_checkin < inicio_checkin:
                print(f"{bcolors.WARNING}"
                      f"Ops, a data que você digitou é anterior à data de "
    ...
    try:
        dao = CheckinDAO()
        checkin = Checkin(inicio=inicio_checkin, final=final_checkin,
                          id_usuario=user.id_, local_id_=selected_local)
        dao.create(checkin)
        system("clear")
        print(
